  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="absolute top-0 right-0 bottom-0 w-[440px] max-w-full bg-white shadow-2xl flex flex-col rounded-l-xl"
      >
        <div className="flex items-center justify-between px-4 py-3 border-b-2 border-gray-100 shrink-0">
          <h3 className="text-sm font-medium text-gray-900 truncate">File Details</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors p-1 rounded hover:bg-gray-100 -mr-1">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3">
          <p className="font-mono text-[11px] text-gray-600 break-all mb-4 leading-relaxed">{file.path}</p>
          ...
          {factors.length > 0 && (
            <div className="mb-4">
              <div className="text-xs text-gray-600 font-medium mb-2">Why is this file risky?</div>
              <div className="space-y-1.5">
                {factors.map((f, i) => (
                  <div
                    key={i}
                    className={`flex items-start gap-2 px-2.5 py-1.5 rounded border ${
                      f.severity === 'critical' ? 'bg-red-50/60 border-red-200' : 'bg-amber-50/60 border-amber-200'
                    }`}
                  >
                    <svg
          ...
          {factors.length === 0 && (
            <div className="mb-4 px-2.5 py-2 rounded border border-green-200/60 bg-green-50/60">
              <p className="text-xs text-green-600">No significant risk factors detected.</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            {metrics.map((m) => (
              <div key={m.label} className="flex justify-between items-center py-1.5 border-b border-gray-50">
